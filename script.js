...
function cargarConfig() {
  ...
  Object.keys(obtenerDatos()).forEach(s => {
    if (s !== "Manual") {
      configSesion.innerHTML += `<option value="${s}">${s}</option>`;
    }
  });
  mostrarTarjetas();
}
...
`.trim();
  ...
  url.searchParams.append("entry.1274396", servicioPersiste.value);
  url.searchParams.append("entry.16222912", perdidaMonitoreo.value);
  url.searchParams.append("entry.288532483", conectadoPor.value);
  url.searchParams.append("entry.763051468", estadoOntPersiste.value);
  url.searchParams.append("entry.1097538933", redesUnificadas.value);
...
function cargarHistorial() {
  ...
  (JSON.parse(localStorage.getItem("historial")) || [])
    .forEach(t => {
      ...
      historialLista.innerHTML += `<div class="historial-item fade-in"><pre>${t}</pre></div>`;
    });
}
...
function cargarLinks() {
  ...
  linksImportantes.forEach(l => {
    linksLista.innerHTML += `<div class="link-item fade-in"><a href="${l.url}" target="_blank">${l.nombre}</a></div>`;
  });
}
